...
extension String {
    ...
    public static func permlinkWith(string: String, isComment: Bool = false) -> String {
        ...
        if isComment {
            if string.hasPrefix("re-") {
                ...
                guard strings.count > 0 else {
                    return perfix + "\(timestamp)" + "-" + "\(string)"
                }
                strings.removeLast()
        ...
        let transform2 = String(transform1.filter {
            latinLettersAndNumbers.contains($0)
        })
    ...
    public func trimSpaces() -> String {
        self.trimmingCharacters(in: .whitespaces).replacingOccurrences(of: " ", with: "")
    }
}
