...
extension Date {
    ...
    public static func defaultTransactionExpiry(expireSeconds: Double = 0.0) -> Date {
        Date(timeIntervalSince1970: (Date().timeIntervalSince1970) + 120.0 + expireSeconds)
    }

    public func seconds(date: Date) -> Int {
        guard let seconds = Calendar.current.dateComponents([.second], from: self, to: date).second, seconds > 0 else {
            return 0
        }
        return seconds
    }
    ...
    public func generateCurrentTimeStamp() -> String {
        String(describing: UInt64(Date().timeIntervalSince1970))
    }
